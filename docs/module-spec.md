...
| `do_run` | _(kdevops-owned, no Yocto analog)_ | Start a workload, monitor, or recording session whose runtime presence is the deliverable. Distinct from `do_deploy`: `do_deploy` lands a long-lived asset, `do_run` activates an ephemeral session that produces observable output. Concrete kdevops examples: capturing a `/sys` or `/proc` start snapshot, kicking off an fio job, activating a libbpf-tools tracer, starting a sysstat recorder. Yocto's lifecycle ends at deploy so no analog exists. |
| `do_collect` | _(kdevops-owned, no Yocto analog)_ | Harvest data produced by a workload or monitor into a results artifact on disk. The inverse of `do_install`: where `do_install` writes the recipe's own built artifacts, `do_collect` writes observed runtime state. Concrete kdevops examples: dumping the end snapshot of a monitor, copying fio output to the results directory, fetching SMART logs to JSON, reading a tracepoint counter delta. Yocto has no analog because Yocto's job ends before runtime. |
