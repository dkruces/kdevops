...
The `ansible_inventory` ansible role is used to let us generate the top level
`inventory` file used as input for ansible for our inventory. The ansible
inventory is a set of nodes we available for our command and control we
will use for command and control.

The `ansible_inventory` ansible role uses jinja2 `template` engine supported by
...
The `ansible_inventory` ansible role is *only* in charge of generating the top level
ansible `inventory` file.

## Dynamic ansible inventory files

Since the number of host we support is dynamic we must also then generate
a host file which is dynamic. The ansible `inventory` file can change per target
...
There is a *super boring* default ansible inventory file generated by default
...
the initial parallelizaiton goals and use the default inventory file. You'd copy
the default hosts template file for your workflow,
`playbooks/roles/ansible_inventory/templates/inventory.j2`. So for example that is what
we have today for CXL, the `playbooks/roles/ansible_inventory/templates/cxl.j2`
is pretty boring today. The only difference right now is that the generic
hosts template file got `NFS` support.

The default boring non-dedicated workflow inventory file:

  * [kdevops dynamic ansible inventory file](playbooks/roles/ansible_inventory/templates/inventory.j2)

## Picking which template to use

The [ansible ansible_inventory main tasks file](playbooks/roles/ansible_inventory/tasks/main.yml)
...
the bool Kconfig option `KDEVOPS_WORKFLOWS_DEDICATED_WORKFLOW`. If however you don't yet
