...
The `nodes` ansible role uses jinja2 `template` engine supported by
...
The `nodes` ansible role is in charge of generating node specific
...
Although the `nodes` ansible role is used to generate the nodes files,
you can override this if you need to. The default settings are set at the
top level Makefile.

```
export KDEVOPS_PLAYBOOKS_DIR := playbooks
KDEVOPS_NODES_ROLE_TEMPLATE_DIR := $(KDEVOPS_PLAYBOOKS_DIR)/roles/nodes/templates
```
