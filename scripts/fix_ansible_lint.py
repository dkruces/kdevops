...
def get_ansible_lint_issues(files: List[str]) -> Dict[str, List[Dict]]:
    ...
    try:
        result = subprocess.run(
            ["ansible-lint", "--format", "json"] + files, capture_output=True, text=True
        )
        if result.stdout:
...
def main():
    ...
    parser.add_argument(
        "--check", action="store_true", help="Run ansible-lint after fixing to verify"
    )
    parser.add_argument(
    ...
    if args.check and not args.dry_run and modified_files:
        ...
        result = subprocess.run(
            ["ansible-lint"] + modified_files, capture_output=True, text=True
        )
