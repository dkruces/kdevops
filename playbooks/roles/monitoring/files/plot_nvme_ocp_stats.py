...
def plot_nvme_ocp_stats(stats_files, output_file):
    ...
    for idx, file in enumerate(stats_files):
        ...
        time_hours = [t * (300/3600) for t in time_hours]  # Convert intervals to hours (default 5min intervals)
        
        # Extract key metrics
        media_written = extract_metric_series(data_points, "Physical media units written")
        media_read = extract_metric_series(data_points, "Physical media units read")
        bad_blocks_raw = extract_metric_series(data_points, "Bad user nand blocks - Raw")
        thermal_events = extract_metric_series(data_points, "Number of Thermal throttling events")
        uncorrectable_errors = extract_metric_series(data_points, "Uncorrectable read error count")
        erase_count_max = extract_metric_series(data_points, "Max User data erase counts")
        free_blocks_pct = extract_metric_series(data_points, "Percent free blocks")
        
        # Plot 1: Media Wear (Written/Read)
        ax1.plot(time_hours, media_written, label=f"{display_name} (Written)", 
                color=color, linewidth=linewidth, linestyle=linestyle, alpha=alpha)
        ax1.plot(time_hours, media_read, label=f"{display_name} (Read)", 
                color=color, linewidth=linewidth, linestyle=':', alpha=alpha*0.8)
        
        # Plot 2: Health Metrics
        ax2.plot(time_hours, bad_blocks_raw, label=f"{display_name} (Bad Blocks)", 
                color=color, linewidth=linewidth, linestyle=linestyle, alpha=alpha)
        ax2_twin = ax2.twinx()
        ax2_twin.plot(time_hours, free_blocks_pct, label=f"{display_name} (Free %)", 
                     color=color, linewidth=linewidth, linestyle='--', alpha=alpha*0.7)
        
        # Plot 3: Thermal and Performance
        ax3.plot(time_hours, thermal_events, label=f"{display_name} (Thermal)", 
                color=color, linewidth=linewidth, linestyle=linestyle, alpha=alpha,
                marker='o' if not is_dev else '^', markersize=4, 
                markevery=max(1, len(time_hours) // 10))
        
        # Plot 4: Reliability Metrics
        ax4.plot(time_hours, uncorrectable_errors, label=f"{display_name} (Uncorrectable)", 
                color=color, linewidth=linewidth, linestyle=linestyle, alpha=alpha)
        ax4_twin = ax4.twinx()
        ax4_twin.plot(time_hours, erase_count_max, label=f"{display_name} (Max Erase)", 
                     color=color, linewidth=linewidth, linestyle='-.', alpha=alpha*0.7)
    ...
    if len(stats_files) == 2 and len(ab_data) == 2:
        ...
        if baseline and dev:
            ...
            written_diff = ((dev["final_written"] - baseline["final_written"]) / baseline["final_written"]) * 100 if baseline["final_written"] > 0 else 0
            read_diff = ((dev["final_read"] - baseline["final_read"]) / baseline["final_read"]) * 100 if baseline["final_read"] > 0 else 0
            ...
            ax1.text(0.02, 0.98, diff_text, transform=ax1.transAxes, fontsize=10,
...
def main():
    ...
    parser.add_argument("stats_files", nargs="+", help="List of *_nvme_ocp_*_stats.txt files")
    parser.add_argument("-o", "--output", default="nvme-ocp-stats.png", 
