...
class TestHandleResult(unittest.TestCase):
    ...
    def test_ok_result_promotes_task_to_completed(self):
        ...
        result.task.action = "ansible.builtin.command"
        result.task._uuid = "uuid-1"
        result.task.get_name.return_value = "test task"
        result.task.get_path.return_value = None
        result.result = {
            "cmd": "echo hi",
            "stdout": "hi",
            "stderr": "",
...
class TestCleanedResult(unittest.TestCase):
    ...
    def test_debug_module_strips_bookkeeping(self):
        ...
        result = MagicMock()
        result.task.action = "ansible.builtin.debug"
        original = {
            "changed": True,
            "invocation": "foo --bar",
            "msg": "hello",
        }
        result.result = copy.deepcopy(original)
...
@ANSIBLE_REQUIRED
class TestVarsPromptNoDeadlock(unittest.TestCase):
    """v2_playbook_on_vars_prompt must not deadlock in dynamic mode.

    output_lock is a non-reentrant threading.Lock; holding it across
    _clear_display (which acquires the same lock) hangs the process
    on any vars_prompt that fires with display_lines > 0.
    """

    def test_vars_prompt_returns_in_dynamic_mode(self):
        cb = _make_callback()
        cb.dynamic_mode = True
        cb.display_lines = 1
        ...
        def _call():
            with patch("sys.stdout"):
                cb.v2_playbook_on_vars_prompt(varname="pw", prompt="enter:")
            finished.set()
        ...
        self.assertTrue(
            finished.is_set(),
            "v2_playbook_on_vars_prompt deadlocked in dynamic mode",
        )


@ANSIBLE_REQUIRED
class TestStatsUnregistersAtexit(unittest.TestCase):
    """v2_playbook_on_stats must drop its atexit cleanup hook.

    Long-lived hosts (ansible-runner reuse) accumulate stale
    registrations otherwise.
    """

    def test_atexit_unregister_called(self):
        cb = _make_callback()
        cb.update_thread_stop = threading.Event()
        cb.update_thread = None
        with patch.object(lucid.atexit, "unregister") as mock_unregister:
            cb.v2_playbook_on_stats(stats=MagicMock())
        mock_unregister.assert_called_once_with(cb._cleanup)
